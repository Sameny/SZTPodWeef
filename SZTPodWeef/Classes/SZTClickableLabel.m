...
#import "UIImage+SZTClickableAdd.h"
...
- (CGRect)containerFrameWithString:(NSString *)string index:(NSUInteger)index font:(UIFont *)font location:(CGPoint)location {
    CGFloat layerHeight = _font.lineHeight + self.textContentInsets.top + self.textContentInsets.bottom;
    CGFloat layerWidth = [string boundingRectWithSize:CGSizeMake(self.bounds.size.width, layerHeight) options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading|NSStringDrawingTruncatesLastVisibleLine) attributes:@{NSFontAttributeName:font} context:nil].size.width + self.textContentInsets.left + self.textContentInsets.right;
...
@implementation NSDictionary (SZTClickableAdd)
